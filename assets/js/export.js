...
export async function exportCSV(state){
  ...
  for(const x of list){ rows.push([x.type,x.date,String(Number(x.amount||0)),x.currency,String(toBase(x.amount, x.currency, config, x.date, x.fxRate)),config.baseCurrency,x.category,x.pay,(x.tags||[]).join("|"),x.notes||""]); }
